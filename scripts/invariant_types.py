...
def _extract_items(source: dict) -> set[str]:
    ...
    if kind == "ast_decorator_functions":
        # Return the canonical identifier for each function decorated with the
        # given decorator. By default this is the Python function name; if
        # `name_kwarg` is set (e.g. "name"), the decorator call's kwarg is
        # used instead when present. fpt-mcp uses @mcp.tool(name="sg_find")
        # on async def sg_find_tool(...) — the public name is "sg_find".
        tree = ast.parse(_read(source["file"]))
                ...
                for deco in node.decorator_list:
                    ...
                    if name_kwarg and isinstance(deco, ast.Call):
                        for kw in deco.keywords:
                            if kw.arg == name_kwarg and isinstance(kw.value, ast.Constant) and isinstance(kw.value.value, str):
