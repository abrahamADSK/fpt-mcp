...
build_index.py
==============
Indexes ShotGrid API documentation into a local ChromaDB vector database.
...
What it indexes:
    - docs/SG_API.md           (shotgun_api3 Python SDK reference + anti-patterns)
    - docs/TK_API.md           (Toolkit sgtk — templates, paths, PipelineConfiguration)
    - docs/REST_API.md         (ShotGrid REST API reference + comparison with Python SDK)
...
that list many methods/operators, further split into groups of
...
_PKG_DIR = _RAG_DIR.parent
INDEX_DIR = str(_RAG_DIR / "index")
DOCS_DIR = str(_PKG_DIR / "docs")

# Documents to index — processed in order
PRIMARY_DOCS = [
    os.path.join(DOCS_DIR, "SG_API.md"),
    os.path.join(DOCS_DIR, "TK_API.md"),
    os.path.join(DOCS_DIR, "REST_API.md"),
]

# Map doc filename to API category for metadata
_API_TAG = {
    "SG_API.md": "shotgun_api3",
    "TK_API.md": "toolkit",
...
def _load_config():
    from fpt_mcp.rag.config import (
        METHOD_GROUP_SIZE,
